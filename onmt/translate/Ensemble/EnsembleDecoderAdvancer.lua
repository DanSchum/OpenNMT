...
function DecoderAdvancer:__init(decoders, batch, contexts, max_sent_length, max_num_unks, decStates, dicts, word_pen,  length_norm, coverage_norm, eos_norm, ensembleOps)
  ...
  -- initialize the decStates for each model
...
function DecoderAdvancer:expand(beam)
  ...
  for i = 1, self.nModels do
		...
		if i == 1 then
			score = self.decoders[i].generator:forward(decOuts[i])
		else
			local newScore = self.decoders[i].generator:forward(decOuts[i])
			for n = 1, #score do
  ...
  local out = score -- average the score 
