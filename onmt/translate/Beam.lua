...
function Beam:_normalizeScores(scores)
  ...
  if self._params.coverage_norm ~= 0 then
    local cumAttnProba = self._state[9]:view(self._remaining, scores:size(2), -1)
...
function Beam:_expandScores(scores, beamSize)
  ...
  if self._params.eos_norm > 0 then
    local EOS_penalty = torch.div(self._state[7]:view(remaining, beamSize), self._step/self._params.eos_norm)
