...
--[[
Implementation of a conditional LSTM step
...
--]]
local CLSTM, parent = torch.class('onmt.CLSTM', 'onmt.Network')
...
--]]
function CLSTM:__init(layers, inputSize, hiddenSize, dropout, residual, dropout_input, attention)
...
function CLSTM:_buildModel(layers, inputSize, hiddenSize, dropout, residual, dropout_input, attention)
  local inputs = {}
  local outputs = {}
  ...
  table.insert(inputs, nn.Identity()()) -- c: batchSize x nSourceSteps x hiddenSize 
  local context = inputs[#inputs]
  ...
  for L = 1, layers do
    ...
    -- we add dropout from the input of first layer
    if dropout > 0 then
			input = nn.Dropout(dropout)(input)
		end
		
		--~ local attentionQuery 
		--~ if L == 1 then
			--~ 
		--~ end
...
function LSTM:_buildLSTMNode(inputSize, hiddenSize, ln)
  local inputs = {}
